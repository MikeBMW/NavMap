...
class TruckRoutePlanActivity : BaseNavActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        navigatorDrive.searchRoute(
            RoutePlanRequester.Companion.newBuilder(NavRouteReqParam.TravelMode.TravelModeDriving)
                .start(NavSearchPoint(40.191873, 116.251917))
                .end(NavSearchPoint(39.715935, 116.462760))
                .options(
                    DriveRoutePlanOptions.Companion.newBuilder()
                        .licenseNumber("京A12345") // 车牌号
                        .truckOptions(
                            DriveRoutePlanOptions.TruckOptions.newBuilder()
                                .setHeight(4f) // 设置货车高度。单位：m
                                .setLength(10f) // 设置货车长度。单位：m
                                .setWidth(4f) // 设置货车宽度。单位：m
                                .setWeight(20f) // 设置货车重量。单位：t
                                .setAxisCount(2) // 设置货车轴数
                                .setAxisLoad(4f) // 设置货车轴重。单位：t
    ...
    private fun initRouteView() {
        routeView = RouteView(this, null)
        routeView!!.injectMap(layerRootDrive.mapApi)
        (findViewById<View>(R.id.app_root_view) as FrameLayout).addView(routeView)
    }
}